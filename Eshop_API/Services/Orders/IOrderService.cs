...
namespace eshop_api.Services.Orders
{
    ...
    {
        Task<List<OrderView>> GetListOrders(bool getDetails); //Lấy danh sách đơn hàng
        ...
        Task<List<OrderView>> GetOrdersByUserId(int userId, bool getDetails); //Lấy đơn hàng theo user id
        Task<List<OrderView>> GetOrdersByStatus(int status, bool getDetails); // Lấy đơn hàng theo trạng thái
        Task<List<OrderView>> GetOrderByStatusOfEachUser(string username, int status, bool getDetails); // Lấy đơn hàng theo trạng thái của mỗi người dùng
